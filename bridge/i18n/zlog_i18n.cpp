...
static const char* gPairs[][2] = {
	...
	{" -> ", " → "},
	{"Unzip failed!\n", "解压失败！\n"},
	{"Unzip cancelled by user.\n", "用户已取消解压。\n"},
	{"Zip: extract cancelled (during file write).\n", "Zip：写入文件时用户已取消解压。\n"},
	{"Zip: extract cancelled (between entries).\n", "Zip：条目之间用户已取消解压。\n"},
	...
	{"Zip: archive cancelled (during file write).\n", "Zip：写入文件时用户已取消打包。\n"},
	{"Zip: archive cancelled (between entries).\n", "Zip：条目之间用户已取消打包。\n"},
	...
	{"Unzip OK!", "解压成功！"},
	{"Archive OK!", "打包成功！"},
	{"Cleaning IPA packaging junk...\n", "正在清理 IPA 中多余无效文件...\n"},
	{"Removed: ", "已删除："},
	{"Cleaned ", "已清理 "},
	{" item(s).\n", " 项。\n"},
	{"Invalid custom remove pattern (skipped): ", "无效的自定义清理规则（已跳过）："},
	{"Multiple .app bundles in Payload, skip custom remove paths.\n", "Payload 中存在多个 .app，已跳过自定义清理规则。\n"},
	...
	{"Verify: code slot ", "校验：代码槽 "},
	{" hash mismatch (hashType=", " 哈希不匹配（hashType="},
	{"Verify: no embedded signature found\n", "校验：未找到嵌入签名\n"},
	{"Verify: invalid SuperBlob magic\n", "校验：无效的 SuperBlob 魔数\n"},
	{"Verify: SuperBlob length exceeds signature region\n", "校验：SuperBlob 长度超出签名区域\n"},
	{"Verify: CodeDirectory has invalid magic\n", "校验：CodeDirectory 魔数无效\n"},
	{"Verify: CodeDirectory codeLimit(", "校验：CodeDirectory codeLimit("},
	{") != actual codeLength(", ") != 实际 codeLength("},
	{"Verify: missing CodeDirectory\n", "校验：缺少 CodeDirectory\n"},
	{"Verify: missing CMS Signature\n", "校验：缺少 CMS 签名\n"},
	{"Verify: CMS blob has invalid magic\n", "校验：CMS blob 魔数无效\n"},
	{"Verify: app folder not set\n", "校验：应用目录未设置\n"},
	{"Verify: no Mach-O files found\n", "校验：未找到 Mach-O 文件\n"},
	{"Verify: can't read main Info.plist\n", "校验：无法读取主 Info.plist\n"},
	{"Verify: can't parse main Info.plist\n", "校验：无法解析主 Info.plist\n"},
	{"Verify: missing CFBundleExecutable\n", "校验：缺少 CFBundleExecutable\n"},
	{"Verify: can't open main binary ", "校验：无法打开主程序 "},
	{"Verify: main binary embedded signature check FAILED: ", "校验：主程序嵌入签名检查失败："},
	...
	/* master 614caa8 增量 */
	{"-r, --bundle_version\tNew bundle version to change.\n", "-r, --bundle_version\t要修改的 Bundle 版本。\n"},
	{"-I, --icon\t\tPath to new app icon to replace the primary icon. (PNG format)\n",
		"-I, --icon\t\t要替换的主图标路径（PNG）。\n"},
	{"-2, --sha256_only\t(Deprecated, now the default.) Kept for backward compatibility.\n",
		"-2, --sha256_only\t（已弃用，现为默认。）保留以兼容旧脚本。\n"},
	{"-L, --legacy_sha1\tEmit a dual SHA1+SHA256 CodeDirectory for iOS <= 10 compatibility.\n",
		"-L, --legacy_sha1\t输出 SHA1+SHA256 双 CodeDirectory（兼容 iOS ≤10）。\n"},
	{"-P, --inject_extensions\tAlso inject -l dylibs into app extensions (PlugIns/Extensions).\n",
		"-P, --inject_extensions\t同时向扩展（PlugIns/Extensions）注入 -l 指定的 dylib。\n"},
	{"Invalid icon file! Only PNG format is supported. ", "无效的图标文件！仅支持 PNG 格式。"},
	{"Install failed to fork ideviceinstaller! ", "安装失败：无法 fork ideviceinstaller！"},
	{"Install failed to wait ideviceinstaller! ", "安装失败：等待 ideviceinstaller 失败！"},
	{"ideviceinstaller install failed! ", "ideviceinstaller 安装失败！"},
	{"Install via ideviceinstaller is only supported on macOS.\n", "仅 macOS 支持通过 ideviceinstaller 安装。\n"},
};
