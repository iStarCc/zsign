	...
	void (^completionHandler)(int status, NSDate* _Nullable expirationDate, NSString* _Nullable error))
{
	...
	std::string strPKeyFile = [key cStringUsingEncoding:NSUTF8StringEncoding];
	std::string strProvFile = [prov cStringUsingEncoding:NSUTF8StringEncoding];
	std::string strPassword = [pass cStringUsingEncoding:NSUTF8StringEncoding];

	__block ZsignCertLoaded loaded = {NULL, NULL};
	char loadError[512] = {0};
	if (!ZsignLoadCheckCertAssets(strPKeyFile.c_str(), strProvFile.c_str(), strPassword.c_str(), &loaded, loadError, sizeof(loadError))) {
		NSString* msg = loadError[0]
	...
	if (!aia) {
		ZsignFreeCheckCertLoaded(&loaded);
