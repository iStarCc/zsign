...
Swift Package（**`ipax-bridge` 分支**），底层为 master `zsign` C++ 核心。主产品 **`ZsignIPAX`** 提供面向 App 的高层签名与 Mach-O API；同包内另含 **`ZsignSwift`**（CLI 对齐的 `Zsign.run`）。
...
1. [集成](#集成)
2. [快速开始](#快速开始)
3. [ZsignIPAX 公开 API](#zsignipax-公开-api)
4. [ZsignSwift（CLI 对齐）](#zsignswiftcli-对齐)
5. [ZsignOptions 选项参考](#zsignoptions-选项参考)
6. [输入类型与执行流程](#输入类型与执行流程)
7. [常见场景示例](#常见场景示例)
8. [日志与回调](#日志与回调)
9. [返回值与错误](#返回值与错误)
10. [平台说明](#平台说明)
11. [与 CLI 对照](#与-cli-对照)
12. [相关文档](#相关文档)
...
```
/path/to/Github/zsign/ipax-bridge
```
...
`import ZsignIPAX` 后，`Zsign` 枚举提供以下高层方法（返回值 `Bool`：`true` 表示操作已接受/成功，`false` 表示失败）。
...
常用参数：`provisionPath`、`p12Path`、`p12Password`、`customIdentifier`、`customName`、`customVersion`、`adhoc`、`removeProvision`、`removeExtensions`、`removeWatchApp`、`enableDocuments`、`minOSVersion`、`zipLevel`（仅 signIPA）、`tempFolderPath`（仅 signIPA）。

均支持 `zh:`、`logHandler`、`completion`。

### Mach-O

| API | 说明 |
|-----|------|
| `Zsign.checkSigned(appExecutable:)` | 是否已签名 |
| `Zsign.injectDyLib(appExecutable:with:weak:)` | 注入 dylib |
| `Zsign.removeDylibs(appExecutable:using:)` | 移除 dylib |
| `Zsign.listDylibs(appExecutable:)` | 列举已加载 dylib |
| `Zsign.changeDylibPath(appExecutable:for:with:)` | 修改 dylib 路径 |

### IPA 解压 / 打包

| API | 说明 |
|-----|------|
| `Zsign.extractIPA(ipaPath:outputFolderPath:...)` | 解压 `.ipa` |
| `Zsign.archiveFolderToIPA(folderPath:outputPath:...)` | 将 `Payload` 目录打包为 IPA |

### 证书检查

| API | 说明 |
|-----|------|
| `Zsign.checkRevokage(provisionPath:p12Path:p12Password:completionHandler:)` | P12 + 描述文件 OCSP 检查（异步） |

`completionHandler` 参数：`(status, expirationDate, errorMessage)`，`status` 含义与 CLI `-C` 一致（0 有效，1 吊销，2 过期等）。
...
签后校验在 `sign` / `signIPA` 内部始终启用（CodeDirectory + CMS 结构及证书检查）。

---

## ZsignSwift（CLI 对齐）

同包产品 **`ZsignSwift`** 提供与 zsign CLI 完全对齐的 `Zsign.run(inputPath:options:)` 及 `ZsignOptions`。若需 `-l` 注入扩展 `-P`、多 provision、`removePaths` 等完整 CLI 选项组合，可 `import ZsignSwift` 使用下列 API。
...
以下示例使用 **`ZsignSwift`**（`import ZsignSwift`）。若只需 App 内签名，可改用 **`ZsignIPAX.signIPA`**，见 [快速开始](#快速开始)。
...
| 项目 | master CLI | ZsignIPAX | ZsignSwift |
|------|------------|-----------|------------|
| 主流程 | `main()` | `sign` / `signIPA` | `Zsign.run()` → `zsignRun()` |
| 选项集 | 全部 long/short opt | 高层参数字段 | `ZsignOptions` 全字段 |
| Mach-O dylib | `-l` / `-D` | 独立 API | `options.dylibs` / `removeDylibs` |
| 版本 / 帮助 | `-v` / `-h` | `version` | `version` / `helpText()` |
| 中文日志 | zlog_i18n | `zh:` + `bridge/i18n` | `run(zh:)` + `bridge/i18n` |
| IPA 多余文件清理 | — | 签名流程内置 | `removePaths` + `bridge/fs_ipa_junk.*` |
| 签后 CMS 校验 | — | `sign` / `signIPA` 内置 | `check` + `bridge/verify_signed_bundle.*` |

### 分支对照

| 分支 | SPM 产品 | 适用场景 |
|------|----------|----------|
| **`ipax-bridge`** | **ZsignIPAX** + ZsignSwift | App 内签名、Mach-O 工具、CLI 对齐 API |
| `swift` | ZsignSwift | 仅 CLI 对齐 `Zsign.run` |
| `master` | — | 原生 C++ CLI |

Core / bridge 更新时，建议将 `swift` 分支 merge 进 `ipax-bridge`，保持 API 与核心同步。
